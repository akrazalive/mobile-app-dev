...
export default function PrincipalDashboard() {
  ...
  const fetchData = async () => {
    ...
    let query
    switch (activeTab) {
      case 'students': query = supabase.from('students').select('*, users(name, email), classes(name), sections(name)'); break
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <div className="flex-1 p-4 lg:p-8">
          <div className="max-w-3xl">
            <button className="w-full lg:w-auto mb-4 bg-purple-600 text-white px-6 py-3 rounded-xl font-semibold flex items-center justify-center gap-2 hover:bg-purple-700 transition">
                  ...
                  <div key={item.id} className="bg-white rounded-xl p-4 border border-gray-100 flex justify-between items-start">
                    <div>
                      <h3 className="font-semibold text-gray-800">
                        {activeTab === 'students' ? item.users?.name :
                         activeTab === 'teachers' ? item.name : item.name}
                      </h3>
                      {activeTab === 'students' && <p className="text-sm text-gray-500 mt-0.5">Roll: {item.roll_number} · {item.classes?.name}-{item.sections?.name}</p>}
